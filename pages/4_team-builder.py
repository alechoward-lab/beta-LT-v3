...
render_page_header("Team Builder", "Build a team of 1-4 heroes and analyze their combined strengths and weaknesses")
...
# Display team at top so user sees their selection first
if st.session_state.team:
    ...
    if st.button("🗑️ Clear Team", key="clear_team_button", width="stretch"):
        st.session_state.team = []
        st.rerun()

    st.markdown("---")

# Tier ranking — placed above the Add Heroes grid so the action button and
# results are visible right under the team summary without scrolling past
# the full hero list.
if "show_tier" not in st.session_state:
    st.session_state.show_tier = False

if st.session_state.team:
    if st.button("🏆 What tier is my team?", type="primary", width="stretch", key="tier_button"):
        st.session_state.show_tier = True

    if st.session_state.show_tier:
        # Apply preset weighting if no villain was selected.
        ...
        balance_stats = {
            name: float(np.mean([heroes[h][idx] for h in st.session_state.team]))
            for idx, name in balance_factors.items()
        }
        bcol1, bcol2, bcol3, bcol4 = st.columns(4)
        bcols = [bcol1, bcol2, bcol3, bcol4]
        ...
        team_stats = [heroes[h] for h in st.session_state.team]
        combined_stats = np.mean(team_stats, axis=0)
        base_team_score = float(np.dot(combined_stats, weighting))
        synergy_multiplier = calculate_team_synergy(
            st.session_state.team, heroes, len(st.session_state.team)
        )
        ...
        for combo in combinations(hero_names, current_team_size):
            combo_stats = np.mean([heroes[h] for h in combo], axis=0)
            ...
            same_size_combinations.append(combo_base_score * (1.0 + combo_synergy))
        ...
        if len(same_size_combinations) > 0:
            team_rank = int(np.sum(same_size_combinations > team_score)) + 1
            total_teams = len(same_size_combinations)
        else:
            team_rank, total_teams = 1, 1

        if team_score >= mean_score + 1.5 * std_score:
            tier, tier_color, tier_text = "S", "red", "Exceptional - Top 5%"
        elif team_score >= mean_score + 0.5 * std_score:
            tier, tier_color, tier_text = "A", "orange", "Excellent - Top 25%"
        elif team_score >= mean_score - 0.5 * std_score:
            tier, tier_color, tier_text = "B", "green", "Good - Average"
        elif team_score >= mean_score - 1.0 * std_score:
            tier, tier_color, tier_text = "C", "blue", "Below Average"
        elif team_score >= mean_score - 1.5 * std_score:
            tier, tier_color, tier_text = "D", "purple", "Weak - Bottom 10%"
        else:
            tier, tier_color, tier_text = "F", "gray", "Very Weak - Bottom 5%"

        tcol1, tcol2 = st.columns([1, 2])
        with tcol1:
            st.markdown(
                f"<h1 style='text-align: center; color: {tier_color};'>{tier}</h1>",
        ...
        hero_scores = [
            {"Hero": h, "Score": f"{float(np.dot(heroes[h], weighting)):.1f}"}
            for h in st.session_state.team
        ]
        st.dataframe(pd.DataFrame(hero_scores), width="stretch", hide_index=True)
        ...
        fig, ax = plt.subplots(
            figsize=(8, 8), subplot_kw=dict(projection="polar"), facecolor="none"
        )
        angles = np.linspace(0, 2 * np.pi, len(radar_factor_names), endpoint=False).tolist()
        ...
        ax.set_xticklabels(radar_factor_names, size=9, color=_txt)
        ax.set_ylim(-6, 6)
        ax.set_yticks([-5, 0, 5])
        ...
        with sw_col1:
            ...
            strengths = [(f, combined_stats[i]) for i, f in enumerate(STAT_NAMES) if combined_stats[i] >= 2]
            if strengths:
        ...
        with sw_col2:
            ...
            weaknesses = [(f, combined_stats[i]) for i, f in enumerate(STAT_NAMES) if combined_stats[i] <= -2]
            if weaknesses:
...
with fmt_col:
    fmt_filter = st.selectbox(
        ...
        ["Current", "Legacy"],
        index=1,
        key="team_builder_fmt_filter",
        label_visibility="collapsed",
    )
if fmt_filter == "Current":
    available_heroes = [h for h in available_heroes if not HERO_LEGACY.get(h, False)]
else:
    ...
    with wave_col:
        wave_filter = st.multiselect(
            "Filter by waves",
            WAVE_ORDER,
            key="team_builder_wave_filter",
            placeholder="All Waves",
        )
    if wave_filter:
        available_heroes = [h for h in available_heroes if HERO_WAVE.get(h) in wave_filter]

# Display heroes in rows of 6 images
cols_per_row = 6
for row_start in range(0, len(available_heroes), cols_per_row):
    ...
    cols = st.columns(cols_per_row)  # Always 6 columns

    for idx, hero in enumerate(hero_row):
        with cols[idx]:
            if hero in hero_image_urls:
                st.image(hero_image_urls[hero], width="stretch")
                if st.button("➕", key=f"add_{hero}", width="stretch"):

import numpy as np
                            #              e, t, cv,s, d, th,re,mi,c, s, br,lg,si,sc,mu
default_heroes = {  
        "Captain Marvel":       np.array([ 5, 4,-1, 3, 4, 2, 4, 1, 0, 3, 0, 0, 5, 1, 1]),
        "Iron Man":             np.array([ 4,-5, 4, 2, 5, 2, 0, 3, 0,-2, 1, 5,-5, 0, 5]),
        "Spider-Man (Peter)":   np.array([ 4, 0, 3, 5, 3,-2, 2, 0, 4, 1, 2, 0, 0, 0, 0]),
        "Black Panther (T'challa)": np.array([ 3,-1, 4, 3, 3, 1, 1, 5, 0, 0, 0, 3,-1, 0, 1]),
        "She-Hulk":             np.array([ 1, 3,-3, 3, 4,-2, 0, 4, 1, 0, 1, 0,-3, 1, 1]),
        "Captain America":      np.array([ 4, 4, 3, 3, 3, 4, 5, 5, 1, 1, 0, 0, 5, 2, 0]),
        "Ms. Marvel":           np.array([ 3,-2, 1, 3, 0, 3, 3, 1, 0, 0, 3, 3, 0, 0, 0]),
        "Thor":                 np.array([ 2,-4,-4, 3, 3,-1,-2, 5,-1,-3, 1, 0, 3, 0, 2]),
        "Black Widow":          np.array([ 2, 0, 3,-2,-4, 3, 2, 3, 4, 0, 0, 0,-5, 4, 1]),
        "Doctor Strange":       np.array([ 4, 4, 6, 4, 2, 5, 3, 0, 5, 5, 4, 3, 3, 5, 4]),
        "Hulk":                 np.array([-3, 5,-2, 4, 4,-5,-5, 3, 0, 0, 1, 0, 2, 0, 0]),
        "Hawkeye":              np.array([ 1,-1, 4,-3, 2, 1,-2, 5, 3,-1, 0, 0,-3, 5, 0]),
        "Spider-Woman":         np.array([ 3, 1, 4, 3, 2, 4, 2, 2, 3, 1, 2, 0,-3, 5, 0]),
                            #              e, t, cv,s, d, th,re,mi,c, s, br,lg,si,sc,mu
        "Ant-Man":              np.array([ 2, 0, 3, 3, 4, 1, 2, 4, 2, 0, 0, 1,-3, 1, 0]),
        "Wasp":                 np.array([ 1, 3, 0, 1, 4, 2, 3, 3, 0, 1, 1, 0,-5, 0, 0]),
        "Quicksilver":          np.array([ 2,-3, 3, 1, 3, 4, 1, 3, 0,-1, 0, 3, 0, 0, 0]),
        "Scarlet Witch":        np.array([ 2, 3, 5, 3, 3, 2, 3, 1, 3, 4, 1, 0,-3, 2, 0]),
        "Star-Lord":            np.array([ 5, 5, 3, 1, 6, 3, 2, 3,-3, 0, 1, 0,-5, 0, 0]),
        "Groot":                np.array([ 0,-3, 2, 4, 3, 2,-2, 2, 0, 2, 0, 1, 2, 0, 2]),
        "Rocket":               np.array([ 3,-1, 0, 1, 0, 2,-1, 4, 0, 0, 1, 1,-3, 0, 1]),
        "Gamora":               np.array([ 2, 5, 3, 1, 3, 5, 4, 3, 0, 0, 1, 0, 5, 0, 0]),
        "Drax":                 np.array([ 2,-3, 4, 2, 4,-1,-5, 3, 0,-5, 1, 3,-2, 0, 3]),
        "Venom (Flash)":        np.array([ 3, 2, 4, 3, 3, 4, 5, 5, 3, 0, 0, 1,-3, 5, 0]),
        "Spectrum":             np.array([ 3, 3, 2, 2, 2, 3,-2, 3, 0, 0, 2, 0,-5, 0, 0]),
        "Adam Warlock":         np.array([ 3,-3, 2, 3, 2, 4,-1, 1, 3,-2, 3, 2,-5, 0, 1]),
        "Nebula":               np.array([ 2, 1, 2, 1,-3, 2, 0, 1, 1, 0, 0, 0,-5, 0, 1]),
        "War Machine":          np.array([ 1,-2, 1, 2, 4, 0,-2, 5, 0,-3, 0, 1,-3, 0, 2]),
        "Valkyrie":             np.array([ 1, 2,-2, 2, 2,-2,-1, 4, 0,-1, 0, 0,-3, 0, 1]),
        "Vision":               np.array([ 3, 3, 3, 3, 4, 6, 5, 2, 1, 0, 1, 0, 0, 1, 0]),
                            #              e, t, cv,s, d, th,re,mi,c, s, br,lg,si,sc,mu
        "Ghost Spider":         np.array([ 3, 3, 3, 3, 3, 2, 2, 2, 4, 1, 1, 0,-3, 0, 0]),
        "Spider-Man (Miles)":   np.array([ 2, 4, 5, 3, 5, 4, 5, 1, 4, 0, 1, 0, 3, 5, 0]),
        "Nova":                 np.array([ 4, 4, 4, 1, 2, 3, 4, 3, 0, 1, 2, 0, 2, 0, 2]),
        "Ironheart":            np.array([ 2,-3, 4, 3, 6, 5, 0, 3, 0, 0, 2, 6,-3, 0, 4]),
        "SP//dr":               np.array([ 2,-1, 5, 3, 3, 5, 0, 1, 1, 0, 2, 2,-5, 0, 1]),
        "Spider-Ham":           np.array([ 6, 2, 4, 5, 2, 4, 5, 2, 4, 0, 2, 1,-1, 5, 0]),
        "Colossus":             np.array([ 1,-1, 5, 6, 4,-2,-2, 2, 2, 0, 1, 1,-3, 4, 1]),
        "Shadowcat":            np.array([ 3, 4, 2, 3, 1, 3, 5, 3, 3, 0, 0, 0,-5, 3, 0]),
        "Cyclops":              np.array([ 2, 0, 5, 3, 4, 4, 5, 3, 0, 2, 2, 1,-3, 0, 0]),
        "Phoenix":              np.array([ 2, 3, 3, 3, 4, 4, 3, 4, 3, 1, 2, 0, 0, 4, 0]),
        "Wolverine":            np.array([ 3, 5, 3, 4, 5, 3, 4, 5, 0, 0, 1, 0, 1, 0, 0]),
        "Storm":                np.array([ 2, 4, 5, 1, 5, 4, 3, 4, 1, 3, 1, 0,-3, 0, 4]),
        "Gambit":               np.array([ 1,-1, 2, 2, 3, 3, 2, 4, 2, 1, 0, 0,-1, 4, 0]),
        "Rogue":                np.array([ 0, 3, 3, 3, 3, 3, 1, 2, 2, 0, 1, 0, 0, 1, 2]),
                            #              e, t, cv,s, d, th,re,mi,c, s, br,lg,si,sc,mu
        "Cable":                np.array([ 2, 3, 4, 3, 3, 5, 5, 2, 3, 3, 2, 4,-5, 0,-4]),
        "Domino":               np.array([ 3,-2, 5, 2, 4, 3, 2, 4, 1, 0, 0, 3,-5, 0, 1]),
        "Psylocke":             np.array([ 4, 4, 4, 1, 1, 5, 4, 3, 5, 0, 2, 0,-3, 5, 0]),
        "Angel":                np.array([ 2, 5, 2, 2, 3, 5, 5, 2, 1, 0, 2, 0,-1, 0, 0]),
        "X-23":                 np.array([ 1, 5, 4, 3, 5, 5, 5, 4, 0, 0, 1, 2,-2, 0, 0]),
        "Deadpool":             np.array([ 1, 5, 5, 5, 5, 5,-3, 2, 1, 3, 1, 0,-1, 1, 0]),
        "Bishop":               np.array([ 5, 2, 4, 4, 5, 3, 3, 3, 0, 0, 1, 1,-3, 0, 0]),
        "Magik":                np.array([ 4, 2, 4, 3, 2, 4, 4, 3, 2, 0, 1, 1,-5, 5, 0]),
        "Iceman":               np.array([ 3, 0, 3, 3, 2, 1,-2, 4, 3, 2, 0, 0, 0, 0, 1]),
        "Jubilee":              np.array([ 3,-1, 4, 0, 2, 4, 3, 3, 4, 1, 0, 1,-1, 5, 1]),
        "Nightcrawler":         np.array([ 1, 2, 3, 4, 1, 3, 4, 4, 1, 2, 0, 0,-1, 1, 2]),
                            #              e, t, cv,s, d, th,re,mi,c, s, br,lg,si,sc,mu
        "Magneto":              np.array([ 3, 3, 3, 4, 3, 4, 5, 4, 2, 0, 0, 1, 3, 0, 1]),
        "Maria Hill":           np.array([ 2, 1, 6, 2, 3, 5, 5, 2, 2, 2, 2, 6,-3, 1, 2]),
        "Nick Fury":            np.array([ 2, 2, 3, 5, 3, 4, 4, 5, 2, 0, 0, 0,-3, 0, 1]),
        "Black Panther (Shuri)": np.array([ 3, 1, 4, 3, 4, 4, 4, 5, 1, 0, 1, 0, 0, 3, 0]),
        "Silk":                 np.array([ 3,-1, 5, 4, 4, 5, 2, 4, 4, 1, 2, 2,-5, 0, 1]),
        "Falcon":               np.array([ 2, 3, 3, 3, 2, 3, 3, 3, 3, 3, 1, 0,-3, 1, 2]),
        "Winter Soldier":       np.array([ 3, 4, 3, 3, 4, 2, 3, 3, 1, 0, 0, 0, 2, 3, 2]),
        "Tigra":                np.array([ 4, 3, 5, 3, 4, 1, 1, 6, 1, 0, 1, 0,-3, 2, 1]),
        "Hulkling":             np.array([ 4, 3, 4, 3, 3, 4, 5, 3, 0, 0, 1, 0,-3, 0, 0]),
        "Hercules":             np.array([ 3,-1, 5, 4, 3, 2, 0, 2, 1, 1, 0, 3,-3, 0, 1]),
        "Wonder Man":           np.array([ 4, 2, 5, 3, 4, 4, 3, 1, 0, 0, 2, 0,-3, 1, 0]),
    }
